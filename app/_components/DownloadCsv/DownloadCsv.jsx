 ...
 */
const DownloadCsv = ({ data, fileName, csvMapping, appendTimestamp, isFetchingAll }) => {
  ...
  console.log("data from download CSV==", data);
  ...
  const convertToCSV = (objArray, addHeader = true) => {
    // check if objArray is empty
    if (objArray.length === 0) {
      return '';
    }
    // remove duplicate values from the array
    objArray = objArray.filter((value, index, self) => self.indexOf(value) === index);
  ...
  return (
    <button
      type="button"
      className={`flex items-center justify-center w-full px-4 py-2 mt-2 text-sm font-medium rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 ${data.length === 0 || isFetchingAll
        ? 'text-gray-400 bg-gray-200 cursor-not-allowed'
        : 'text-white bg-black hover:bg-gray-800 focus:ring-gray-500'
        }`}
      onClick={downloadCSV}
      disabled={data.length === 0 || isFetchingAll}
    >
